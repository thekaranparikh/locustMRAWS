import tkinter as tk
from tkinter import messagebox
import logic_multi

speed = 50

# ── Bot colour palette — each bot gets a distinct colour ─────────────────────
BOT_COLOURS = ["#3498db", "#e67e22", "#9b59b6", "#1abc9c",
               "#e74c3c", "#f1c40f", "#2ecc71", "#e91e63"]

# ── Block colour palette — each block gets a distinct colour ─────────────────
BLOCK_COLOURS = ["#e74c3c", "#e67e22", "#f39c12", "#f1c40f", 
                 "#2ecc71", "#1abc9c", "#3498db", "#9b59b6",
                 "#e91e63", "#c0392b", "#d35400", "#27ae60"]

class GridPlatform:
    def __init__(self, root):
        self.root = root
        self.root.title("Swarm Architect Pro — Multi-Bot")
        try: self.root.state('zoomed')
        except: pass
        self.root.configure(bg="#1e1e1e")

        self.rows, self.cols, self.grid_size = 20, 20, 40
        self.bg_color, self.grid_color = "#1e1e1e", "#444444"
        self.fit_screen_var = tk.BooleanVar(value=False)

        self.is_simulating = False
        self.initial_state = []
        self.undo_stack    = []
        self.redo_stack    = []
        self.selected_tool = None
        self.tool_color    = None
        self.last_vertex   = None
        self.occupancy_map = {}

        # Track which colour index to assign to the next bot/block placed
        self.bot_colour_index = 0
        self.block_colour_index = 0

        # Stats
        self.step_count   = 0
        self.turn_count   = 0
        self.tick_count   = 0
        self.last_path_dir = None
        self.path_history  = []

        # Assignment display: bot_id -> label widget (updated live)
        self.assignment_labels = {}

        self.setup_ui()
        self.draw_grid()
        self.bind_shortcuts()

    # ── STATS ─────────────────────────────────────────────────────────────────

    def update_stats_display(self):
        self.step_label.config(text=f"STEPS:  {self.step_count}")
        self.turn_label.config(text=f"TURNS:  {self.turn_count}")
        self.tick_label.config(text=f"TICKS:  {self.tick_count}")

    def reset_counters(self):
        self.step_count = 0
        self.turn_count = 0
        self.tick_count = 0
        self.last_path_dir = None
        self.path_history  = []
        self.update_stats_display()

    def add_path_stats(self, dx, dy):
        self.step_count += 1
        current_dir = (int(dx), int(dy))
        if self.last_path_dir and self.last_path_dir != current_dir:
            self.turn_count += 1
        self.last_path_dir = current_dir
        self.path_history.append(current_dir)
        self.update_stats_display()

    def subtract_path_stats(self):
        if self.step_count > 0:
            self.step_count -= 1
            if len(self.path_history) >= 2 and self.path_history[-1] != self.path_history[-2]:
                self.turn_count = max(0, self.turn_count - 1)
            if self.path_history:
                self.path_history.pop()
                self.last_path_dir = self.path_history[-1] if self.path_history else None
            self.update_stats_display()

    # ── ASSIGNMENT PANEL ──────────────────────────────────────────────────────

    def rebuild_assignment_panel(self):
        """Rebuild the per-bot assignment rows from scratch."""
        for w in self.assign_inner.winfo_children():
            w.destroy()
        self.assignment_labels.clear()

        bots = self._collect_bots_from_canvas()
        for bot in bots:
            bid = bot['id']
            colour = self._bot_colour(bid)
            row = tk.Frame(self.assign_inner, bg="#2d2d2d")
            row.pack(fill='x', pady=1)
            tk.Label(row, text="●", fg=colour, bg="#2d2d2d",
                     font=('Arial', 10, 'bold')).pack(side='left')
            lbl = tk.Label(row, text="idle", fg="#888", bg="#2d2d2d",
                           font=('Courier', 8), anchor='w')
            lbl.pack(side='left', fill='x', expand=True)
            self.assignment_labels[bid] = lbl

    def update_assignment_display(self, assignments):
        """
        assignments: dict bot_id -> {'block_pos': (x,y), 'goal_pos': (x,y)}
                     or None if bot is idle / done.
        Called every tick from run_logic_loop.
        """
        for bid, lbl in self.assignment_labels.items():
            info = assignments.get(bid)
            if info is None:
                lbl.config(text="idle", fg="#555")
            elif info.get('done'):
                lbl.config(text="done ✓", fg="#2ecc71")
            else:
                bx = int(info['block_pos'][0] // self.grid_size)
                by = int(info['block_pos'][1] // self.grid_size)
                gx = int(info['goal_pos'][0]  // self.grid_size)
                gy = int(info['goal_pos'][1]  // self.grid_size)
                lbl.config(text=f"B({bx},{by})→G({gx},{gy})", fg="#f1c40f")

    # ── UNDO / REDO ───────────────────────────────────────────────────────────

    def undo(self, event=None):
        if not self.undo_stack or self.is_simulating: return
        action, data, pos = self.undo_stack.pop()
        if action == "add":
            tags = self.canvas.gettags(data) if isinstance(data, str) else ()
            if "path" in tags: self.subtract_path_stats()
            items_to_save = [self.get_item_data(i) for i in self.canvas.find_withtag(data)]
            self.redo_stack.append(("add", items_to_save, pos))
            self.canvas.delete(data)
            if pos in self.occupancy_map: del self.occupancy_map[pos]
        elif action == "delete":
            new_ids = [self.recreate_item(d) for d in data]
            tags = self.canvas.gettags(new_ids[0])
            if "path" in tags:
                c  = self.canvas.coords(new_ids[0])
                self.add_path_stats((c[2]-c[0])/self.grid_size, (c[3]-c[1])/self.grid_size)
            group_tag = next((t for t in tags if t.startswith("group_")), None)
            if group_tag and isinstance(pos, tuple): self.occupancy_map[pos] = group_tag
            self.redo_stack.append(("delete", group_tag or new_ids[0], pos))

    def redo(self, event=None):
        if not self.redo_stack or self.is_simulating: return
        action, data, pos = self.redo_stack.pop()
        if action == "add":
            new_ids = [self.recreate_item(d) for d in data]
            tags = self.canvas.gettags(new_ids[0])
            if "path" in tags:
                c = self.canvas.coords(new_ids[0])
                self.add_path_stats((c[2]-c[0])/self.grid_size, (c[3]-c[1])/self.grid_size)
            group_tag = next((t for t in tags if t.startswith("group_")), None)
            if group_tag and isinstance(pos, tuple): self.occupancy_map[pos] = group_tag
            self.undo_stack.append(("add", group_tag or new_ids[0], pos))
        elif action == "delete":
            items_to_save = [self.get_item_data(i) for i in self.canvas.find_withtag(data)]
            if items_to_save and "path" in items_to_save[0]['tags']: self.subtract_path_stats()
            self.undo_stack.append(("delete", items_to_save, pos))
            self.canvas.delete(data)
            if isinstance(pos, tuple) and pos in self.occupancy_map: del self.occupancy_map[pos]

    def bind_shortcuts(self):
        self.root.bind("<Control-z>", self.undo); self.root.bind("<Control-Z>", self.undo)
        self.root.bind("<Control-y>", self.redo); self.root.bind("<Control-Y>", self.redo)

    # ── SIMULATION ────────────────────────────────────────────────────────────

    def _bot_colour(self, bot_id):
        """Return a consistent colour for a given bot_id string."""
        # Stable hash so colour doesn't change between ticks
        idx = hash(bot_id) % len(BOT_COLOURS)
        return BOT_COLOURS[idx]

    def _collect_bots_from_canvas(self):
        bots = []
        seen = set()
        for item in self.canvas.find_all():
            tags = self.canvas.gettags(item)
            if "bot" in tags and "body" in tags:
                group_tag = next((t for t in tags if t.startswith("group_")), None)
                if group_tag and group_tag not in seen:
                    seen.add(group_tag)
                    coords = self.canvas.coords(item)
                    center = ((coords[0]+coords[2])/2, (coords[1]+coords[3])/2)
                    bots.append({'id': group_tag, 'pos': center})
        return bots

    def rotate_bot_visual(self, bot_tag, dx, dy):
        items = self.canvas.find_withtag(bot_tag)
        h_id = next((i for i in items if "heading" in self.canvas.gettags(i)), None)
        b_id = next((i for i in items if "body"    in self.canvas.gettags(i)), None)
        if h_id and b_id:
            c  = self.canvas.coords(b_id)
            cx, cy = (c[0]+c[2])/2, (c[1]+c[3])/2
            s  = self.grid_size // 3
            hc = self.canvas.coords(h_id)
            cvx, cvy = hc[2]-hc[0], hc[3]-hc[1]
            tvx = (dx/self.grid_size)*s
            tvy = (dy/self.grid_size)*s
            if abs(cvx+tvx) < 1 and abs(cvy+tvy) < 1:          # 180°
                self.canvas.coords(h_id, cx, cy, cx-cvy, cy+cvx)
                self.turn_count += 1; self.update_stats_display()
                self.root.after(150, lambda: self._finish_180(h_id, cx, cy, tvx, tvy))
            else:
                self.canvas.coords(h_id, cx, cy, cx+tvx, cy+tvy)
                self.turn_count += 1; self.update_stats_display()

    def _finish_180(self, h_id, cx, cy, tx, ty):
        self.canvas.coords(h_id, cx, cy, cx+tx, cy+ty)
        self.turn_count += 1; self.update_stats_display()

    def run_logic_loop(self):
        if not self.is_simulating: return
        try:
            self._run_tick()
        except Exception as e:
            import traceback
            print("ERROR in run_logic_loop — keeping simulation alive:")
            traceback.print_exc()
            self.root.after(3000, self.run_logic_loop)

    def _run_tick(self):
        # ──────────────────────────────────────────────────────────────────────
        # ARCHITECTURE:
        #   Logic runs once per full move cycle. Between logic calls the UI may
        #   insert turn ticks. All bots act SIMULTANEOUSLY each tick.
        #
        #   _pending: list of FRAMES. Each frame is a list of actions for ALL
        #   bots that happen in the same tick:
        #       [{'type':'turn','bot_id':...,'to':...}, ...]   ← all bots turn
        #       [{'type':'move','bot_id':...,'dx':...,...}, ...] ← all bots move
        #
        #   Rule: bots that need 2 turns get frame[0]=turn1, frame[1]=turn2,
        #   frame[2]=move. Bots that need 1 turn get frame[0]=turn, frame[1]=move.
        #   Bots already facing correctly go straight to move in frame[0].
        #   Within each frame, every bot is at the same "turn depth" as the others.
        # ──────────────────────────────────────────────────────────────────────

        G = self.grid_size

        def _set_arrow(bot_id, d):
            items = self.canvas.find_withtag(bot_id)
            h_id = next((i for i in items if "heading" in self.canvas.gettags(i)), None)
            b_id = next((i for i in items if "body"    in self.canvas.gettags(i)), None)
            if h_id and b_id:
                c = self.canvas.coords(b_id)
                cx_, cy_ = (c[0]+c[2])/2, (c[1]+c[3])/2
                s = G // 3
                self.canvas.coords(h_id, cx_, cy_, cx_ + d[0]*s, cy_ + d[1]*s)

        def _next_90(frm, to):
            if frm == to: return None
            if frm[0]*to[0] + frm[1]*to[1] == 0: return to
            return (frm[1], -frm[0])   # 180° apart — clockwise 90° first

        # ── Execute the next pending frame (all bots simultaneously) ──────────
        if self._pending:
            frame = self._pending.pop(0)
            frame_has_move = False
            frame_has_turn = False
            for action in frame:
                if action['type'] == 'turn':
                    bot_id = action['bot_id']
                    to_dir = action['to']
                    _set_arrow(bot_id, to_dir)
                    self._bot_facing[bot_id] = to_dir
                    frame_has_turn = True

                elif action['type'] == 'move':
                    bot_id   = action['bot_id']
                    dx, dy   = action['dx'], action['dy']
                    block_id = action['block_id']
                    new_dir  = (dx // G, dy // G)
                    self.canvas.move(bot_id, dx, dy)
                    if block_id:
                        self.canvas.move(block_id, dx, dy)
                    _set_arrow(bot_id, new_dir)
                    self._bot_facing[bot_id] = new_dir
                    frame_has_move = True

            # Increment tick counter once per frame (all actions in frame happen simultaneously)
            self.tick_count += 1
            
            # Count steps: number of bots that moved
            if frame_has_move:
                bots_moved = sum(
                    1 for action in frame
                    if action['type'] == 'move'
                )
                self.step_count += bots_moved
            
            # Count turns: number of bots that turned
            if frame_has_turn:
                bots_turned = sum(
                    1 for action in frame
                    if action['type'] == 'turn'
                )
                self.turn_count += bots_turned
            
            self.update_stats_display()
            self.root.after(speed, self.run_logic_loop)
            return

        # ── _pending empty — build world state and call logic ─────────────────
        state       = {'bots': [], 'blocks': [], 'goals': []}
        seen_groups = set()
        for item in self.canvas.find_all():
            tags = self.canvas.gettags(item)
            if "grid" in tags or "sudo" in tags or "path" in tags: continue
            coords = self.canvas.coords(item)
            if not coords: continue
            cx  = (coords[0] + coords[-2]) / 2
            cy  = (coords[1] + coords[-1]) / 2
            grp = next((t for t in tags if t.startswith("group_")), None)
            if "bot" in tags and "body" in tags:
                if grp:
                    state['bots'].append({'id': grp, 'pos': (cx, cy)})
            elif "block" in tags:
                if grp and grp not in seen_groups:
                    seen_groups.add(grp)
                    state['blocks'].append({'id': grp, 'pos': (cx, cy)})
            elif "goal" in tags:
                if grp and grp not in seen_groups:
                    seen_groups.add(grp)
                    state['goals'].append({'id': grp, 'pos': (cx, cy)})

        result      = logic_multi.compute_swarm_moves(state, self.grid_size)
        moves       = result['moves']
        assignments = result['assignments']
        self.update_assignment_display(assignments)

        # ── Build frames: group per-bot turn/move sequences by depth ─────────
        # For each bot, compute the sequence of actions needed:
        #   e.g. bot A: [turn1, turn2, move]
        #        bot B: [move]
        #        bot C: [turn1, move]
        # Then zip across bots by depth so all bots at depth 0 form frame 0,
        # all at depth 1 form frame 1, etc.
        # Bots that finish early (already at move) just contribute their move
        # at the appropriate depth — bots that need fewer turns wait at their
        # move action until the slowest bot finishes turning.
        #
        # Simpler rule: all bots take the same number of frames.
        # Bots that need fewer turns have their move repeated in later frames? No.
        # Better: each frame contains ONE action per active bot (turn or move).
        # Bots that already have their move queued just wait — they don't act
        # until their frame arrives.

        any_action = False
        # per_bot_sequence[bot_id] = [action, action, ...] in order
        per_bot = {}

        for bot_id, (dx, dy, block_id) in moves.items():
            if dx == 0 and dy == 0:
                continue
            any_action = True
            new_dir  = (dx // G, dy // G)
            prev_dir = self._bot_facing.get(bot_id, (1, 0))

            seq = []
            cur = prev_dir
            while cur != new_dir:
                step = _next_90(cur, new_dir)
                seq.append({'type': 'turn', 'bot_id': bot_id, 'to': step})
                cur = step
            seq.append({'type': 'move', 'bot_id': bot_id,
                        'dx': dx, 'dy': dy, 'block_id': block_id})
            per_bot[bot_id] = seq

        if not any_action:
            blocks = state['blocks']
            goals  = state['goals']
            goal_positions = [g['pos'] for g in goals]
            all_done = (not blocks or not goals or all(
                any(abs(b['pos'][0]-gp[0]) < 10 and abs(b['pos'][1]-gp[1]) < 10
                    for gp in goal_positions)
                for b in blocks
            ))
            if all_done:
                self.toggle_play()
                return
            self._idle_ticks += 1
            if self._idle_ticks > 30:
                self.toggle_play()
                return
            self.root.after(speed, self.run_logic_loop)
            return

        self._idle_ticks = 0

        # Zip sequences into frames: frame[i] = [action_for_botA, action_for_botB, ...]
        # Each bot contributes one action per frame at its depth.
        # If a bot's sequence is shorter, it doesn't appear in later frames.
        max_depth = max(len(seq) for seq in per_bot.values())
        for i in range(max_depth):
            frame = []
            for bot_id, seq in per_bot.items():
                if i < len(seq):
                    frame.append(seq[i])
            if frame:
                self._pending.append(frame)

        self.root.after(200, self.run_logic_loop)

    def confirm_clear(self):
        if messagebox.askyesno("Confirm Clear", "Are you sure?"):
            self.canvas.delete("all")
            self.draw_grid()
            self.occupancy_map.clear()
            self.undo_stack.clear()
            self.redo_stack.clear()
            self.reset_counters()
            self.bot_colour_index = 0
            self.block_colour_index = 0
            self.rebuild_assignment_panel()

    def toggle_play(self):
        if self.is_simulating:
            self.is_simulating = False
            self.play_btn.config(text="▶ PLAY LOGIC", bg="#27ae60")
        else:
            self.initial_state = [
                self.get_item_data(i) for i in self.canvas.find_all()
                if "grid" not in self.canvas.gettags(i)
            ]
            self.canvas.delete("path")
            self.reset_counters()
            logic_multi.reset_logic()
            self._idle_ticks  = 0
            self._pending     = []   # queue of {'type':'turn'|'move', ...} actions

            # Seed _bot_facing from each bot's heading arrow on canvas
            self._bot_facing = {}
            G = self.grid_size
            for item in self.canvas.find_all():
                tags = self.canvas.gettags(item)
                if "bot" not in tags or "heading" not in tags:
                    continue
                grp = next((t for t in tags if t.startswith("group_")), None)
                if not grp:
                    continue
                coords = self.canvas.coords(item)
                if len(coords) < 4:
                    continue
                vx = coords[2] - coords[0]
                vy = coords[3] - coords[1]
                if abs(vx) >= abs(vy):
                    self._bot_facing[grp] = (1 if vx >= 0 else -1, 0)
                else:
                    self._bot_facing[grp] = (0, 1 if vy > 0 else -1)

            self.rebuild_assignment_panel()
            self.is_simulating = True
            self._idle_ticks = 0          # reset stuck-tick counter
            self.play_btn.config(text="⏹ STOP", bg="#c0392b")
            self.run_logic_loop()

    # ── UI SETUP ──────────────────────────────────────────────────────────────

    def setup_ui(self):
        # ── Sidebar ───────────────────────────────────────────────────────────
        self.sidebar = tk.Frame(self.root, width=210, bg="#2d2d2d", padx=10, pady=10)
        self.sidebar.pack(side='left', fill='y')
        self.sidebar.pack_propagate(False)

        self.play_btn = tk.Button(
            self.sidebar, text="▶ PLAY LOGIC", command=self.toggle_play,
            bg="#27ae60", fg="white", font=('Arial', 9, 'bold'))
        self.play_btn.pack(fill='x', pady=5)

        tk.Button(self.sidebar, text="🔄 RESET ARENA",  command=self.reset_simulation,   bg="#2980b9", fg="white").pack(fill='x', pady=2)
        tk.Button(self.sidebar, text="Resize Grid",      command=self.open_resize_dialog, bg="#444",    fg="white").pack(fill='x', pady=2)
        tk.Button(self.sidebar, text="🗑️ DELETE TOOL",   command=self.set_delete_tool,    bg="#e67e22", fg="white").pack(fill='x', pady=5)
        tk.Button(self.sidebar, text="CLEAR ALL",        command=self.confirm_clear,       bg="#c0392b", fg="white").pack(fill='x', pady=2)

        # ── Solid assets ──────────────────────────────────────────────────────
        tk.Label(self.sidebar, text="SOLID ASSETS", fg="white", bg="#2d2d2d",
                 font=('Arial', 9, 'bold')).pack(pady=(10, 4))
        # BOT button - color assigned on placement, not on button click
        tk.Button(self.sidebar, text="BOT", bg="#3498db", fg="white",
                  command=lambda: self.set_tool("BOT", None)).pack(fill='x', pady=2)
        # BLOCK button - uses fixed red color
        tk.Button(self.sidebar, text="BLOCK", bg="#e74c3c", fg="white",
                  command=lambda: self.set_tool("BLOCK", "#e74c3c")).pack(fill='x', pady=2)
        tk.Button(self.sidebar, text="GOAL", bg="#2ecc71", fg="white",
                  command=lambda: self.set_tool("GOAL", "#2ecc71")).pack(fill='x', pady=2)

        # ── Sudo assets ───────────────────────────────────────────────────────
        tk.Label(self.sidebar, text="SUDO ASSETS", fg="#aaa", bg="#2d2d2d",
                 font=('Arial', 9, 'bold')).pack(pady=(10, 4))
        for name, col in [("SUDO BOT", "#3498db"), ("SUDO BLOCK", "#e74c3c")]:
            tk.Button(self.sidebar, text=name, bg=col, fg="white",
                      command=lambda n=name, c=col: self.set_tool(n, c)).pack(fill='x', pady=2)

        # ── Assignment panel ──────────────────────────────────────────────────
        assign_frame = tk.LabelFrame(
            self.sidebar, text=" BOT ASSIGNMENTS ",
            bg="#2d2d2d", fg="#3498db", font=('Arial', 9, 'bold'),
            padx=5, pady=5)
        assign_frame.pack(fill='x', pady=(12, 0))
        self.assign_inner = tk.Frame(assign_frame, bg="#2d2d2d")
        self.assign_inner.pack(fill='x')
        tk.Label(self.assign_inner, text="(place bots first)",
                 fg="#555", bg="#2d2d2d", font=('Arial', 8)).pack()

        # ── Live stats ────────────────────────────────────────────────────────
        stats_frame = tk.LabelFrame(
            self.sidebar, text=" LIVE STATS ",
            bg="#2d2d2d", fg="#f1c40f", font=('Arial', 9, 'bold'),
            padx=8, pady=8)
        stats_frame.pack(side='bottom', fill='x', pady=(10, 0))

        self.step_label = tk.Label(
            stats_frame, text="STEPS:  0",
            bg="#1a1a2e", fg="#00e5ff",
            font=('Courier', 13, 'bold'), anchor='w', padx=6, pady=4)
        self.step_label.pack(fill='x', pady=(0, 4))

        self.turn_label = tk.Label(
            stats_frame, text="TURNS:  0",
            bg="#1a1a2e", fg="#ff6b35",
            font=('Courier', 13, 'bold'), anchor='w', padx=6, pady=4)
        self.turn_label.pack(fill='x', pady=(0, 4))

        self.tick_label = tk.Label(
            stats_frame, text="TICKS:  0",
            bg="#1a1a2e", fg="#9b59b6",
            font=('Courier', 13, 'bold'), anchor='w', padx=6, pady=4)
        self.tick_label.pack(fill='x')

        tk.Button(
            stats_frame, text="↺  RESET STATS",
            command=self.reset_counters,
            bg="#2d2d2d", fg="#888", font=('Arial', 8),
            relief='flat', cursor='hand2').pack(fill='x', pady=(6, 0))

        # ── Canvas ────────────────────────────────────────────────────────────
        self.canvas = tk.Canvas(self.root, bg=self.bg_color, highlightthickness=0)
        self.canvas.pack(side='right', expand=True, fill='both')
        self.canvas.bind("<Button-1>",        self.on_left_click)
        self.canvas.bind("<Button-3>",        self.on_right_click)
        self.canvas.bind("<B3-Motion>",       self.on_right_drag)
        self.canvas.bind("<ButtonRelease-3>", lambda e: setattr(self, 'last_vertex', None))

    def _next_bot_colour(self):
        """Return the next colour from the bot palette (cycles if > 8 bots)."""
        col = BOT_COLOURS[self.bot_colour_index % len(BOT_COLOURS)]
        self.bot_colour_index += 1
        return col

    def _next_block_colour(self):
        """Return the next colour from the block palette (cycles if > 12 blocks)."""
        col = BLOCK_COLOURS[self.block_colour_index % len(BLOCK_COLOURS)]
        self.block_colour_index += 1
        return col

    # ── CLICK / DRAG ──────────────────────────────────────────────────────────

    def on_left_click(self, event):
        if self.is_simulating: return
        cx, cy = self.canvas.canvasx(event.x), self.canvas.canvasy(event.y)

        if self.selected_tool == "DELETE":
            found = self.canvas.find_overlapping(cx-2, cy-2, cx+2, cy+2)
            for item in found:
                tags = self.canvas.gettags(item)
                if "grid" in tags: continue
                if "path" in tags:
                    self.subtract_path_stats()
                    self.undo_stack.append(("delete", [self.get_item_data(item)], item))
                    self.redo_stack.clear()
                    self.canvas.delete(item)
                    return
                group_tag = next((t for t in tags if t.startswith("group_")), None)
                if group_tag:
                    items_to_del = self.canvas.find_withtag(group_tag)
                    c = self.canvas.coords(items_to_del[0])
                    obj_pos = (
                        round(((c[0]+c[-2])/2) / self.grid_size) * self.grid_size,
                        round(((c[1]+c[-1])/2) / self.grid_size) * self.grid_size,
                    )
                    self.undo_stack.append(("delete",
                                            [self.get_item_data(i) for i in items_to_del],
                                            obj_pos))
                    self.redo_stack.clear()
                    self.canvas.delete(group_tag)
                    keys = [k for k, v in self.occupancy_map.items() if v == group_tag]
                    for k in keys: del self.occupancy_map[k]
                    return
            return

        # Placement
        snap_x = round(cx / self.grid_size) * self.grid_size
        snap_y = round(cy / self.grid_size) * self.grid_size
        s = self.grid_size // 3
        if (snap_x - s < 0 or snap_x + s > self.cols * self.grid_size or
                snap_y - s < 0 or snap_y + s > self.rows * self.grid_size):
            return

        pos = (snap_x, snap_y)
        if self.selected_tool:
            if pos in self.occupancy_map: return
            obj_id = self.place_item(snap_x, snap_y, self.selected_tool, self.tool_color)
            if obj_id:
                self.occupancy_map[pos] = obj_id
                self.undo_stack.append(("add", obj_id, pos))
                self.redo_stack.clear()

    def place_item(self, x, y, tool, color):
        s     = self.grid_size // 3
        g_tag = f"group_{x}_{y}_{tk.Tcl().call('clock', 'clicks')}"

        if tool == "BOT":
            # Assign color at placement time (not button click time)
            if color is None:
                color = self._next_bot_colour()
            self.canvas.create_oval(x-s, y-s, x+s, y+s,
                                    fill=color, outline="white",
                                    tags=("bot", "asset", g_tag, "body"))
            self.canvas.create_line(x, y, x+s, y,
                                    fill="yellow", width=2, arrow=tk.LAST,
                                    tags=("bot", "asset", g_tag, "heading"))
            return g_tag

        elif tool == "BLOCK":
            # Use fixed red color
            self.canvas.create_rectangle(x-s, y-s, x+s, y+s,
                                         fill=color, outline="white",
                                         tags=("block", "asset", g_tag))
            return g_tag

        elif tool == "GOAL":
            self.canvas.create_line(x-s, y-s, x+s, y+s,
                                    fill=color, width=3,
                                    tags=("goal", "asset", g_tag))
            self.canvas.create_line(x+s, y-s, x-s, y+s,
                                    fill=color, width=3,
                                    tags=("goal", "asset", g_tag))
            return g_tag

        elif "SUDO" in tool:
            tag   = "sudo_bot" if "BOT" in tool else "sudo_block"
            shape = self.canvas.create_oval if "BOT" in tool else self.canvas.create_rectangle
            shape(x-s, y-s, x+s, y+s,
                  fill="", outline=color, width=2, dash=(4, 4),
                  tags=(tag, "sudo", "asset", g_tag))
            return g_tag

        return None

    def on_right_click(self, event):
        cx, cy = self.canvas.canvasx(event.x), self.canvas.canvasy(event.y)
        found = self.canvas.find_overlapping(cx-2, cy-2, cx+2, cy+2)
        for item in found:
            tags = self.canvas.gettags(item)
            if "bot" in tags:
                group_tag = next((t for t in tags if t.startswith("group_")), None)
                if group_tag:
                    self.cycle_bot_orientation(group_tag)
                    self.last_vertex = None
                    return
        sv = (round(cx / self.grid_size) * self.grid_size,
              round(cy / self.grid_size) * self.grid_size)
        if (0 <= sv[0] <= self.cols * self.grid_size and
                0 <= sv[1] <= self.rows * self.grid_size):
            self.last_vertex = sv
        else:
            self.last_vertex = None

    def on_right_drag(self, event):
        if not self.last_vertex: return
        cx, cy = self.canvas.canvasx(event.x), self.canvas.canvasy(event.y)
        curr_v = (round(cx / self.grid_size) * self.grid_size,
                  round(cy / self.grid_size) * self.grid_size)
        if not (0 <= curr_v[0] <= self.cols * self.grid_size and
                0 <= curr_v[1] <= self.rows * self.grid_size):
            return
        if curr_v != self.last_vertex:
            adx = abs(curr_v[0] - self.last_vertex[0])
            ady = abs(curr_v[1] - self.last_vertex[1])
            if (adx == self.grid_size and ady == 0) or (adx == 0 and ady == self.grid_size):
                dx = (curr_v[0] - self.last_vertex[0]) / self.grid_size
                dy = (curr_v[1] - self.last_vertex[1]) / self.grid_size
                self.add_path_stats(dx, dy)
                g_tag = f"group_arrow_{self.last_vertex[0]}_{self.last_vertex[1]}_{tk.Tcl().call('clock', 'clicks')}"
                self.canvas.create_line(
                    self.last_vertex[0], self.last_vertex[1],
                    curr_v[0], curr_v[1],
                    fill="yellow", width=3, arrow=tk.LAST,
                    tags=("path", "asset", g_tag))
                self.undo_stack.append(("add", g_tag, None))
                self.redo_stack.clear()
                self.last_vertex = curr_v

    def cycle_bot_orientation(self, bot_tag):
        items = self.canvas.find_withtag(bot_tag)
        h_id  = next((i for i in items if "heading" in self.canvas.gettags(i)), None)
        b_id  = next((i for i in items if "body"    in self.canvas.gettags(i)), None)
        if h_id and b_id:
            c  = self.canvas.coords(b_id)
            cx, cy = (c[0]+c[2])/2, (c[1]+c[3])/2
            hc = self.canvas.coords(h_id)
            vx, vy = hc[2]-hc[0], hc[3]-hc[1]
            self.canvas.coords(h_id, cx, cy, cx-vy, cy+vx)
            logic_multi.reset_logic()

    # ── GRID / RESIZE ────────────────────────────────────────────────────────

    def draw_grid(self):
        self.canvas.delete("grid")
        w, h = self.cols * self.grid_size, self.rows * self.grid_size
        for i in range(self.cols + 1):
            self.canvas.create_line(i*self.grid_size, 0, i*self.grid_size, h,
                                    fill=self.grid_color, tags="grid")
        for j in range(self.rows + 1):
            self.canvas.create_line(0, j*self.grid_size, w, j*self.grid_size,
                                    fill=self.grid_color, tags="grid")
        self.canvas.tag_lower("grid")

    def open_resize_dialog(self):
        dialog = tk.Toplevel(self.root)
        dialog.title("Resize Arena")
        dialog.configure(bg="#2d2d2d")
        tk.Label(dialog, text="Columns:", fg="white", bg="#2d2d2d").grid(row=0, column=0, padx=10, pady=5)
        e_cols = tk.Entry(dialog); e_cols.insert(0, str(self.cols)); e_cols.grid(row=0, column=1, padx=10, pady=5)
        tk.Label(dialog, text="Rows:",    fg="white", bg="#2d2d2d").grid(row=1, column=0, padx=10, pady=5)
        e_rows = tk.Entry(dialog); e_rows.insert(0, str(self.rows)); e_rows.grid(row=1, column=1, padx=10, pady=5)
        tk.Checkbutton(dialog, text="Fit to Screen", variable=self.fit_screen_var,
                       bg="#2d2d2d", fg="white", selectcolor="#2d2d2d",
                       activebackground="#2d2d2d").grid(row=2, columnspan=2, pady=5)
        tk.Button(dialog, text="APPLY", bg="#27ae60", fg="white",
                  command=lambda: self.apply_resize(e_rows.get(), e_cols.get(), dialog)
                  ).grid(row=3, columnspan=2, pady=10)

    def apply_resize(self, r, c, win):
        try:
            new_rows, new_cols = int(r), int(c)
            if new_rows < 1 or new_cols < 1: return
            if self.fit_screen_var.get():
                self.root.update_idletasks()
                cw = self.canvas.winfo_width()
                ch = self.canvas.winfo_height()
                if cw <= 1: cw = self.root.winfo_screenwidth()  - 230
                if ch <= 1: ch = self.root.winfo_screenheight() - 60
                self.grid_size = max(10, min((cw-40)//new_cols, (ch-40)//new_rows))
            self.rows, self.cols = new_rows, new_cols
            self.canvas.delete("all")
            self.draw_grid()
            self.occupancy_map.clear()
            self.undo_stack.clear()
            self.redo_stack.clear()
            self.reset_counters()
            win.destroy()
        except ValueError:
            messagebox.showerror("Error", "Please enter valid integers.")

    # ── SIMULATION RESET ─────────────────────────────────────────────────────

    def reset_simulation(self):
        if self.is_simulating:
            self.is_simulating = False
            self.play_btn.config(text="▶ PLAY LOGIC", bg="#27ae60")
        self.canvas.delete("all")
        self.draw_grid()
        self.occupancy_map.clear()
        self.reset_counters()
        for d in self.initial_state:
            new_id = self.recreate_item(d)
            tags   = d.get('tags', [])
            if "asset" in tags and "path" not in tags:
                c = d['coords']
                center = (
                    round(((c[0]+c[-2])/2) / self.grid_size) * self.grid_size,
                    round(((c[1]+c[-1])/2) / self.grid_size) * self.grid_size,
                )
                self.occupancy_map[center] = new_id
        logic_multi.reset_logic()
        self.rebuild_assignment_panel()

    # ── SERIALISATION ────────────────────────────────────────────────────────

    def get_item_data(self, i):
        return {
            'type':   self.canvas.type(i),
            'coords': self.canvas.coords(i),
            'fill':   self.canvas.itemcget(i, "fill"),
            'width':  self.canvas.itemcget(i, "width"),
            'tags':   self.canvas.gettags(i),
        }

    def recreate_item(self, d):
        t, coords, fill, width, tags = d['type'], d['coords'], d['fill'], d['width'], d['tags']
        is_sudo  = "sudo" in tags
        is_arrow = "path" in tags or "heading" in tags
        dash     = (4, 4) if is_sudo else ()
        outline  = fill if is_sudo else "white"
        if t == 'line':
            return self.canvas.create_line(*coords, fill=fill, width=width,
                                           tags=tags, arrow=tk.LAST if is_arrow else "")
        elif t == 'oval':
            return self.canvas.create_oval(*coords, fill=fill, outline=outline,
                                           width=width, tags=tags, dash=dash)
        elif t == 'rectangle':
            return self.canvas.create_rectangle(*coords, fill=fill, outline=outline,
                                                width=width, tags=tags, dash=dash)

    # ── TOOL SELECTION ───────────────────────────────────────────────────────

    def set_tool(self, t, c): self.selected_tool, self.tool_color = t, c
    def set_delete_tool(self): self.selected_tool = "DELETE"


if __name__ == "__main__":
    root = tk.Tk()
    app  = GridPlatform(root)
    root.mainloop()